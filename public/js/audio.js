...
class SoundEngine {
  ...
  // 9. MATH QUIZ: CORRECT ANSWER CHIME (Cheerful bell chord)
  playCorrect() {
    if (this.isMuted) return;
    this.init();
    const t = this.ctx.currentTime;
  ...
  // 10. MATH QUIZ: WRONG ANSWER BUZZ/THUD
  playWrong() {
    if (this.isMuted) return;
    this.init();
    const t = this.ctx.currentTime;
  ...
  playCombo(combo = 1) {
    ...
    this.init();
    const t = this.ctx.currentTime;
    const baseFreq = 440 * Math.pow(1.059463, Math.min(combo * 2, 16));
    const osc = this.ctx.createOscillator();
    const gain = this.ctx.createGain();
    osc.type = 'triangle';
  ...
  // 13. CANNON LOAD / READY SOUND
  playLoadCannon() {
    if (this.isMuted) return;
    this.init();
    const t = this.ctx.currentTime;
    // Heavy metal latch sliding shut
    const osc = this.ctx.createOscillator();
    const gain = this.ctx.createGain();
    osc.type = 'sawtooth';
